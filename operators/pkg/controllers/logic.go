...
func (r *LabInstanceReconciler) CreateEnvironment(labInstance *crownlabsv1alpha2.Instance, labTemplate *crownlabsv1alpha2.Template, namespace string, name string, VMstart time.Time) error {
	...
	secret := instanceCreation.CreateCloudInitSecret(name, namespace, user, password, r.NextcloudBaseUrl)
	...
	vmi, err := instanceCreation.CreateVirtualMachineInstance(name, namespace, labTemplate.Spec.EnvironmentList[0], labInstance.Name, secret.Name)
	if err != nil {
		return err
	}
